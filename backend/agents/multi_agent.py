...
구조 (LangGraph):
  START
    └─▶ supervisor  ─ 도구 선택 + 소스별 최적화 검색쿼리 생성
          ...
          ├─▶ inquiry_node          ─ Chroma inquiry_cases (국세청 질의회신 119,427건)
...
from db.chroma_search import (
    ...
    search_inquiry_cases as _chroma_inquiry,
)
...
def supervisor_node(state: MultiAgentState) -> dict:
    ...
    # 도구는 항상 7개 모두 사용 — LLM에게 선택권 주지 않음
    tools = [
        "search_cases", "search_taxlaw_prec", "search_taxtr",
        "search_law", "search_inquiry", "search_issue_cache", "search_pdf_cases",
    ]
    ...
    if _is_itcl_query(state["query"]):
        ...
        inquiry_query = "국제조세 이전가격 정상가격 질의회신 국외특수관계인 법인세 과세"
    else:
        ...
        try:
            ...
            inquiry_query = plan_data.get("inquiry_query") or state["query"]
        except Exception:
            law_query = prec_query = taxtr_query = inquiry_query = state["query"]
...
_VALID_TOOLS = {
    ...
    "search_taxtr", "search_inquiry", "search_issue_cache", "search_pdf_cases",
}
...
# ── 6. Inquiry Cases Search (Chroma inquiry_cases, 국세청 질의회신 119,427건) ──

def inquiry_node(state: MultiAgentState) -> dict:
    query = state.get("inquiry_query") or state["query"]
    results = _chroma_inquiry(query, n=6)
    return {
        "inquiry_results": results,
        "done_tools": ["search_inquiry"],
    }


# ── 7. PDF Court Cases Search (Chroma pdf_court_cases) ──────────────────────
...
def synthesizer_node(state: MultiAgentState) -> dict:
    ...
    inquiry_block = _fmt_inquiry_cases(state.get("inquiry_results") or [])
    ...
    has_inquiry = bool(state.get("inquiry_results"))
...
def _fmt_inquiry_cases(items: list) -> str:
    if not items:
        return "(질의회신 결과 없음)"
    lines = []
    for it in items[:5]:
        doc_no = it.get("doc_no", "")
        tax_type = it.get("tax_type", "")
        reply_date = it.get("reply_date", "")
        title = it.get("title") or (it.get("document") or "")[:60]
        doc = (it.get("document") or "")[:200]
        sim = it.get("similarity", 0)
        lines.append(
            f"• [{doc_no}] {tax_type} | 회신일:{reply_date} | 유사도:{sim:.2f}\n"
            f"  {title}\n  {doc}"
        )
    return "\n".join(lines) or "(없음)"
...
def _build_graph():
    ...
    g.add_node("search_inquiry", inquiry_node)
    ...
    g.add_edge("search_inquiry", "supervisor")
...
class SupervisorAgent:
    ...
    def run(self, query: str, messages: list = []) -> dict:
        ...
        return {
            ...
            "inquiry_cases_context": final.get("inquiry_results") or [],
